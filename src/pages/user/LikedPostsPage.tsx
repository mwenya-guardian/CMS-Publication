...
import { RefreshCcwDot, Calendar, Quote as QuoteIcon, Rss, AudioLines } from 'lucide-react';
...
export const LikedPostsPage: React.FC = () => {
  ...
  const loadData = async (isLoadMore = false) => {
    ...
    try {
      if (activeTab === 'posts') {
        const postsData = await postService.getAllLikedPeginated(postsPage, 5);
        ...
        const eventsData = await eventService.getAllLikedPaginated(eventsPage, 5);
        ...
        const quotesData = await quoteService.getAllLikedPaginated(quotesPage, 5);
        ...
        const publicationsData = await publicationService.getAllLikedPaginated(publicationsPage, 5);
  ...
  const handleReactionChange = (entityId: string) => {
    // Remove the entity from the appropriate list if it's no longer liked
    if (activeTab === 'posts') {
      setPosts(prevPosts => prevPosts.filter(post => post.id !== entityId));
    } else if (activeTab === 'events') {
      setEvents(prevEvents => prevEvents.filter(event => event.id !== entityId));
    } else if (activeTab === 'quotes') {
      setQuotes(prevQuotes => prevQuotes.filter(quote => quote.id !== entityId));
    } else if (activeTab === 'publications') {
      setPublications(prevPublications => prevPublications.filter(publication => publication.id !== entityId));
    }
  };
        ...
        >
            ...
            <div className="space-y-4">
              {posts.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-gray-500">No liked posts available at the moment.</p>
                </div>
              ) : (
                posts.map((post) => (
                  <PostCard
                    key={post.id}
                    post={post}
                    onReactionChange={() => handleReactionChange(post.id)}
                  />
                ))
              )}
            </div>
          ...
          {activeTab === 'events' && (
            <div className="space-y-4">
              {events.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-gray-500">No liked events available at the moment.</p>
                </div>
              ) : (
                events.map((event) => (
                  <ReactionWrapper
                    ...
                    initialDislikes={0}
                    initialComments={0}
                    onReactionChange={() => handleReactionChange(event.id)}
                  >
                    <div className="p-4">
          ...
          {activeTab === 'quotes' && (
            <div className="space-y-4">
              {quotes.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-gray-500">No liked quotes available at the moment.</p>
                </div>
              ) : (
                quotes.map((quote) => (
                  <ReactionWrapper
                    ...
                    initialDislikes={0}
                    initialComments={0}
                    onReactionChange={() => handleReactionChange(quote.id)}
                  >
                    <div className="p-4">
          ...
          {activeTab === 'publications' && (
            <div className="space-y-4">
              {publications.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-gray-500">No liked publications available at the moment.</p>
                </div>
              ) : (
                publications.map((publication) => (
                  <ReactionWrapper
                    ...
                    initialDislikes={0}
                    initialComments={0}
                    onReactionChange={() => handleReactionChange(publication.id)}
                  >
                    <div className="p-4">
              ...
              {!hasMoreData[activeTab] && (
                <div className="text-center py-4">
                  <p className="text-gray-500 mb-3">No more liked content available</p>
                  <button
