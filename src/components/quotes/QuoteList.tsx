import React, { useState, useEffect } from 'react';
import { Quote } from '../../types/Quote';
import { QuoteCard } from './QuoteCard';
import { GridLayout } from '../layoutOptions/GridLayout';
import { ListLayout } from '../layoutOptions/ListLayout';
import { MasonryLayout } from '../layoutOptions/MasonryLayout';
import { LoadingSpinner } from '../common/LoadingSpinner';
import { Button } from '../common/Button';
import { Grid, List, Grid3X3, Quote as QuoteIcon } from 'lucide-react';
import { FilterOptions, LayoutType } from '../../types/Common';

interface QuoteListProps {
  quotes: Quote[];
  isLoading?: boolean;
  isAdmin?: boolean;
  onEdit?: (quote: Quote) => void;
  onDelete?: (id: string) => void;
  onView?: (quote: Quote) => void;
  filters?: FilterOptions;
  layoutType?: LayoutType;
  onLayoutChange?: (layout: LayoutType) => void;
  className?: string;
}

export const QuoteList: React.FC<QuoteListProps> = ({
  quotes,
  isLoading = false,
  isAdmin = false,
  onEdit,
  onDelete,
  onView,
  layoutType = 'grid',
  onLayoutChange,
  className = '',
}) => {
  const [groupedQuotes, setGroupedQuotes] = useState<{ [year: string]: Quote[] }>({});

  useEffect(() => {
    const grouped = quotes.reduce((acc, quote) => {
      console.warn(quote?.createdAt);
      const year = new Date(quote.createdAt).getFullYear().toString();
      // console.warn(year)
      if (!acc[year]) {
        acc[year] = [];
      }
      acc[year].push(quote);
      return acc;
    }, {} as { [year: string]: Quote[] });

    // Sort quotes within each year by date (newest first)
    Object.keys(grouped).forEach(year => {
      grouped[year].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    });

    setGroupedQuotes(grouped);
  }, [quotes]);

  const renderQuotes = (quoteList: Quote[]) => {
    const cards = quoteList.map((quote) => (
      <QuoteCard
        key={quote.id}
        quote={quote}
        layoutType={layoutType}
        isAdmin={isAdmin}
        onEdit={onEdit}
        onDelete={onDelete}
        onView={onView}
      />
    ));

    switch (layoutType) {
      case 'list':
        return <ListLayout>{cards}</ListLayout>;
      case 'masonry':
        return <MasonryLayout columns={3}>{cards}</MasonryLayout>;
      case 'grid':
      default:
        return <GridLayout columns={3}>{cards}</GridLayout>;
    }
  };

  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12">
        <LoadingSpinner size="lg" />
      </div>
    );
  }

  if (quotes.length === 0) {
    return (
      <div className="text-center py-12">
        <div className="text-gray-400 mb-4">
          <QuoteIcon className="mx-auto h-12 w-12" />
        </div>
        <h3 className="text-lg font-medium text-gray-900 mb-2">No quotes found</h3>
        <p className="text-gray-600">
          {isAdmin ? 'Create your first quote to get started.' : 'No quotes match your current filters.'}
        </p>
      </div>
    );
  }

  const years = Object.keys(groupedQuotes).sort((a, b) => parseInt(b) - parseInt(a));

  return (
    <div className={className}>
      {onLayoutChange && (
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-gray-900">
            {quotes.length} Quote{quotes.length !== 1 ? 's' : ''}
          </h2>
          <div className="flex items-center space-x-2">
            <span className="text-sm text-gray-600 mr-2">Layout:</span>
            <div className="flex items-center space-x-1 bg-gray-100 rounded-lg p-1">
              <Button
                variant={layoutType === 'grid' ? 'primary' : 'ghost'}
                size="sm"
                onClick={() => onLayoutChange('grid')}
                icon={Grid}
              />
              <Button
                variant={layoutType === 'list' ? 'primary' : 'ghost'}
                size="sm"
                onClick={() => onLayoutChange('list')}
                icon={List}
              />
              <Button
                variant={layoutType === 'masonry' ? 'primary' : 'ghost'}
                size="sm"
                onClick={() => onLayoutChange('masonry')}
                icon={Grid3X3}
              />
            </div>
          </div>
        </div>
      )}

      <div className="space-y-12">
        {years.map((year) => (
          <div key={year}>
            <div className="flex items-center mb-6">
              <h3 className="text-xl font-semibold text-gray-900">{year}</h3>
              <div className="ml-4 flex-1 h-px bg-gray-200" />
              <span className="ml-4 text-sm text-gray-600">
                {groupedQuotes[year].length} quote{groupedQuotes[year].length !== 1 ? 's' : ''}
              </span>
            </div>
            {renderQuotes(groupedQuotes[year])}
          </div>
        ))}
      </div>
    </div>
  );
};