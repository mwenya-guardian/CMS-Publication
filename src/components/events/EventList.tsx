import React, { useState, useEffect } from 'react';
import { Event } from '../../types/Event';
import { EventCard } from './EventCard';
import { GridLayout } from '../layoutOptions/GridLayout';
import { ListLayout } from '../layoutOptions/ListLayout';
import { LoadingSpinner } from '../common/LoadingSpinner';
import { Button } from '../common/Button';
import { Grid, List, Calendar } from 'lucide-react';
import { FilterOptions, LayoutType } from '../../types/Common';

interface EventListProps {
  events: Event[];
  isLoading?: boolean;
  isAdmin?: boolean;
  onEdit?: (event: Event) => void;
  onDelete?: (id: string) => void;
  onView?: (event: Event) => void;
  filters?: FilterOptions;
  layoutType?: LayoutType;
  onLayoutChange?: (layout: LayoutType) => void;
  className?: string;
}

export const EventList: React.FC<EventListProps> = ({
  events,
  isLoading = false,
  isAdmin = false,
  onEdit,
  onDelete,
  onView,
  layoutType = 'grid',
  onLayoutChange,
  className = '',
}) => {
  const [groupedEvents, setGroupedEvents] = useState<{ [year: string]: Event[] }>({});

  useEffect(() => {
    const grouped = events.reduce((acc, event) => {
      const year = new Date(event.startDate).getFullYear().toString();
      if (!acc[year]) {
        acc[year] = [];
      }
      acc[year].push(event);
      return acc;
    }, {} as { [year: string]: Event[] });

    // Sort events within each year by date (newest first)
    Object.keys(grouped).forEach(year => {
      grouped[year].sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime());
    });

    setGroupedEvents(grouped);
  }, [events]);

  const renderEvents = (evts: Event[]) => {
    const cards = evts.map((event) => (
      <EventCard
        key={event.id}
        event={event}
        layoutType={layoutType === 'masonry' ? 'grid' : layoutType}
        isAdmin={isAdmin}
        onEdit={onEdit}
        onDelete={onDelete}
        onView={onView}
      />
    ));

    if (layoutType === 'list') {
      return <ListLayout>{cards}</ListLayout>;
    } else {
      return <GridLayout columns={3}>{cards}</GridLayout>;
    }
  };

  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12">
        <LoadingSpinner size="lg" />
      </div>
    );
  }

  if (events.length === 0) {
    return (
      <div className="text-center py-12">
        <div className="text-gray-400 mb-4">
          <Calendar className="mx-auto h-12 w-12" />
        </div>
        <h3 className="text-lg font-medium text-gray-900 mb-2">No events found</h3>
        <p className="text-gray-600">
          {isAdmin ? 'Create your first event to get started.' : 'No events match your current filters.'}
        </p>
      </div>
    );
  }

  const years = Object.keys(groupedEvents).sort((a, b) => parseInt(b) - parseInt(a));

  return (
    <div className={className}>
      {onLayoutChange && (
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-gray-900">
            {events.length} Event{events.length !== 1 ? 's' : ''}
          </h2>
          <div className="flex items-center space-x-2">
            <span className="text-sm text-gray-600 mr-2">Layout:</span>
            <div className="flex items-center space-x-1 bg-gray-100 rounded-lg p-1">
              <Button
                variant={layoutType === 'grid' ? 'primary' : 'ghost'}
                size="sm"
                onClick={() => onLayoutChange('grid')}
                icon={Grid}
              />
              <Button
                variant={layoutType === 'list' ? 'primary' : 'ghost'}
                size="sm"
                onClick={() => onLayoutChange('list')}
                icon={List}
              />
            </div>
          </div>
        </div>
      )}

      <div className="space-y-12">
        {years.map((year) => (
          <div key={year}>
            <div className="flex items-center mb-6">
              <h3 className="text-xl font-semibold text-gray-900">{year}</h3>
              <div className="ml-4 flex-1 h-px bg-gray-200" />
              <span className="ml-4 text-sm text-gray-600">
                {groupedEvents[year].length} event{groupedEvents[year].length !== 1 ? 's' : ''}
              </span>
            </div>
            {renderEvents(groupedEvents[year])}
          </div>
        ))}
      </div>
    </div>
  );
};